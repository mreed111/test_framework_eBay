...
public class UI_vaidate_footer_links {
...
//    }
    ...
    @Test
    public void TC_validateFooterLinks() {
        ...
        for ( int i=0; i<linksCount; i++ )
        {
        	...
        	if (nextLinkTitle.equals("eBay country sites"))
    		{
        		...
        		//Assert.assertEquals("verify mouse over on \""+nextLinkTitle+"\"", "true", footerLinks.get(i).getAttribute("aria-expanded"));
    			if (footerLinks.get(i).getAttribute("aria-expanded").equals("false"))
    			{
    				Results = Results + "," + nextLinkTitle;
    			} 
        		tb.driver.navigate().refresh();
    		} 
    		else if (nextLink.length() > 0)
        	{
        		// verify link
        		String linkResult = tb.verifyLinkByPageTitle(footerLinks.get(i), this.titleFirstPage);
        		Results = (linkResult.length() > 0) ? Results + "," + linkResult : Results;
        	}
        }
        
        if (Results.length() > 0)
        {
        	Assert.fail("Links Failed: "+Results);
        }
